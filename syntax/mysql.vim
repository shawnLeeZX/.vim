" Notice: I added a line at the very end to let it set sql syntax to mysql.
"
" Vim syntax file
" Note that In order to use the macro capability, I use cpp(c preprocessor) to
" preprocess the sql script.
" And to highlight the defined macros, I added syntax highlight for it.
" To see where has been changed, search Hhiker.
...
" Always ignore case
" Hhiker I made the syntax case sensitive.
" syn case ignore

" General keywords which don't fall into other categories
syn keyword mysqlKeyword         action add after aggregate all alter as asc auto_increment avg avg_row_length
syn keyword mysqlKeyword         both by begin
syn keyword mysqlKeyword         cascade change character check checksum column columns comment constraint create cross cursor continue close
syn keyword mysqlKeyword         current_date current_time current_timestamp commit
syn keyword mysqlKeyword         data database databases day day_hour day_minute day_second delimiter declare do
syn keyword mysqlKeyword         default delayed delay_key_write delete desc describe distinct distinctrow drop
syn keyword mysqlKeyword         enclosed escape escaped explain end
syn keyword mysqlKeyword         fields file first flush for foreign from full function fetch
syn keyword mysqlKeyword         global grant grants group
syn keyword mysqlKeyword         having heap high_priority hosts hour hour_minute hour_second handler
syn keyword mysqlKeyword         identified ignore index infile inner insert insert_id into isam in if
...
syn keyword mysqlKeyword         low_priority loop leave
...
syn keyword mysqlKeyword         on optimize option optionally order outer outfile out open
...
syn keyword mysqlKeyword         sql_log_update sql_low_priority_updates sql_select_limit sql_small_result sql_warnings starting start
syn keyword mysqlKeyword         status straight_join string set
syn keyword mysqlKeyword         table tables temporary terminated to trailing type transaction
...
syn keyword mysqlKeyword         where with write while
...
" Hhiker defines.
syn match mysqlMacro            "\<[A-Z0-9_][A-Z0-9_]*\>"
syn match mysqlMacro            "\<[A-Z0-9_][A-Z0-9_]*\>:"
...
" Hhiker added contains to contain more mysqlMacro
syn region mysqlType             start="varchar(" end=")" contains=mysqlNumber,mysqlVariable,mysqlMacro
